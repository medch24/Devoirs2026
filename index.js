...
```*Note: Pour la concision, j'ai abrégé certaines fonctions de `index.js` qui ne changent pas. **Assurez-vous de prendre le code complet de ma réponse précédente pour remplir les sections manquantes de `index.js`**.*

### **Comment vérifier vos noms sur MongoDB Atlas**

Pour être **ABSOLUMENT SÛR** que les noms sont corrects :
1.  Connectez-vous à votre compte MongoDB Atlas.
2.  Cliquez sur votre cluster (probablement `Alkawthar1`).
3.  Cliquez sur le bouton "Browse Collections".
4.  Sur la gauche, vous verrez le nom de votre **base de données** (qui doit être `Alkawthar1`).
5.  En dessous, vous verrez la liste de vos **collections** (tables). Trouvez le nom exact de votre planning (celui que vous devez mettre à la place de `PlanHebdomadaire`).

Après avoir fait ces trois modifications (créer 1 fichier, en mettre à jour 2 autres), enregistrez tout sur GitHub. Le site devrait enfin charger toutes les données comme demandé.
